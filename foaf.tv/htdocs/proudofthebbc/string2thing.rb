...
#print open('http://www.domain.com/path/page.rhtml').read
...
wiki_qs = "http://ajax.googleapis.com/ajax/services/search/web?v=1.0&q=zzzzz%20site:wikipedia.org"
 bbc_qs = "http://ajax.googleapis.com/ajax/services/search/web?v=1.0&q=zzzzz%20site:www.bbc.co.uk/programmes/"
...
f.each do |item|
  ...
  #
  wres = open(wiki_qs.gsub(/zzzzz/, esc) ).read 
...
#  bres =` #{ bbc_qs.gsub(/zzzzz/, esc) } ` 
  bres = open(bbc_qs.gsub(/zzzzz/, esc) ).read 
